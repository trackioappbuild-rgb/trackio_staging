...
export function AdminShell({ active, children }: { active: string; children: ReactNode }) {
  return <div className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark"><i/><i/></span><strong>trackio</strong></div><div className="academy"><span className="academy-icon">♙</span><div><strong>Aegis Academy</strong><small>Administrator</small></div><span>⌄</span></div><p className="menu-label">MAIN MENU</p><nav>{links.map(([icon,label,href])=><Link className={label===active?"active":""} href={href} key={label}><span className="nav-icon">{icon}</span><span>{label}</span>{label==="Alerts"&&<b className="badge">3</b>}</Link>)}</nav><div className="user-card"><span className="avatar">SJ</span><div><strong>Sarah Johnson</strong><small>Transport Manager</small></div><span>⋮</span></div><button className="collapse">← <span>Collapse Menu</span></button></aside><div className="workspace"><header className="topbar"><button className="mobile-menu">☰</button><div className="search"><span>⌕</span><span>Search students, parents, routes, buses...</span><kbd>⌘ K</kbd></div><div className="top-actions"><button>♧<b>3</b></button><button>▱</button><button>⚙</button><span className="mini-avatar">SJ</span></div></header>{children}</div></div>;
}
